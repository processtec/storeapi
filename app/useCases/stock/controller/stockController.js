...
const getStock = async (req, res) => {
  ...
  if (!Number.isInteger(stockId)) {
    return res.send(
      errorRes("stock id should be an integar!", "path", "stack", "code")
    );
  }
...
const getProductsByComponentAndStock = async (req, res) => {
  ...
  if (!Number.isInteger(stockId) || !Number.isInteger(cmpId)) {
    return res.send(
      errorRes(
        "stockId or cmpId should be an integar!",
        "path",
        "stack",
        "code"
      )
...
const getStockByComponent = async (req, res) => {
  ...
  if (!Number.isInteger(cmpId)) {
    return res.send(
      errorRes("cmpId  should be an integar!", "path", "stack", "code")
    );
  }
...
const addProduct = async (req, res) => {
  const { errMsg, params } = validateAddParams(req.body);
...
const sellProduct = async (req, res) => {
  const { errMsg, params } = validateSellParams(req.body);
