...
const ocSyncWorkerStart = async () => {
  ...
  if (lastTimeStamp === null) {
    logger.error(
      "OC sync dont need it now as we have a basic ES sync we use Logstash for that."
    );
...
const syncPTEOC = async () => {
  let lastSync = await syncService.lastOCSyncTimeStamp();

  if (lastSync === null || lastSync === undefined) {
    lastSync = {
      syncedTimestamp: "2021-01-06 17:51:12",
      OrderConfirmationID: 1000,
    };
  }
  ...
  if (pendingOCsInPTE === null || pendingOCsInPTE === undefined) {
    logger.error(
