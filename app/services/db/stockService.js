...
const addProductTx = async (options) => {
  ...
  trials 6*/
    ...
    if (options.status === SConst.PRODUCT.STATUS.AVAILABLE) {
      ...
      const updateStockResult = await updateStockForAddingProduct(
        connection,
        options
      );
    } else if (options.status === SConst.PRODUCT.STATUS.ORDERED) {
      options.lastModifiedBy = options.lastModifiedBy || "in Future.";
      const updateStockResult = await updateStockForOrderingProduct(
        connection,
        options
      );
    }
...
const addProductsTx = async (products) => {
  ...
  try {
    ...
    logger.debug(
      ...
      "products added and commited to DB."
    );
  }
};
...
const createStockIfRequired = async (connection, options) => {
  // 0. check if stock for this componenet already exisits.
  const stockSearchResult = await findOneStockByComponent(connection, options);
  let stock = stockSearchResult;
  if (!(stockSearchResult && _.has(stockSearchResult, "idstock"))) {
    //create a new stock with cmp if stock entry not present.
    const { idstock } = await create(connection, options);
    const stockSearchResult = await findOneStockByComponent(
      connection,
      options
    );
    ...
    if (!(stockSearchResult && _.has(stockSearchResult, "idstock"))) {
      throw new Error(
        "unable to create a stock for cmponent: " + options.cmpId
      );
    }
...
const addNewProduct = async (connection, options) => {
  ...
  try {
    const [
      ...
      "INSERT INTO product SET idcmp = ?, idsupplier = ?, idvendor = ?, mpin = ?, status = ?, costprice = ?, saleprice = ?, idpo = ?, idstock = ?, idlocation = ?, PurchaseOrderCode = ?",
      [
        options.cmpId,
        options.idSupplier,
        options.idVendor,
...
const updateStockForAddingProduct = async (connection, options) => {
  ...
  const availablequantity =
    options.stock.availablequantity > 0
      ? options.stock.availablequantity + 1
      : 1;
  ...
  try {
    const [
      ...
      "UPDATE stock SET availablequantity = ?, lastModifiedBy = ?, price = ?, mfgmodelnumber = ?, idsupplier = ? where idstock = ?",
      [
        availablequantity,
...
const updateStockForOrderingProduct = async (connection, options) => {
  ...
  const orderedquantity =
    options.stock.orderedquantity > 0 ? options.stock.orderedquantity + 1 : 1;
  ...
  try {
    const [
      ...
      [orderedquantity, options.lastModifiedBy, options.stock.idstock]
    );
    result = rows;
    logger.info(
      {
...
const findOneStockByComponent = async (connection, options) => {
  ...
  try {
    const [
      ...
      "SELECT * from stock where idcmp = ? LIMIT 0, 1",
      [options.cmpId]
    );
...
const create = async (connection, options) => {
  ...
  try {
    const [
      ...
      "INSERT INTO stock SET idcmp = ?, price = ?, mfgmodelnumber = ?, idsupplier = ?",
      [options.cmpId, options.costPrice, options.idSupplier, options.idSupplier]
    );
    result = rows;
    logger.info(
      {
...
const getAvailableProductsForStockId = async (options) => {
  ...
  try {
    ...
    const [
      ...
      "SELECT prod.idproduct, prod.idcmp, prod.idsupplier, prod.idvendor, prod.mpin, prod.serialno, prod.idtag, prod.status, prod.costprice, prod.saleprice, prod.idpo, prod.idoc, prod.idstock, prod.supplier_company, prod.vendor_company, loca.name, loca.level, loca.zone, loca.rack FROM store.product as prod left join store.location as loca ON loca.idlocation = prod.idlocation where prod.idstock = ? AND prod.idcmp = ? AND prod.status = ? AND prod.isActive = 1 ORDER BY prod.createdOn ASC",
      [options.stockId, options.cmpId, SConst.PRODUCT.STATUS.AVAILABLE]
    );
    result = rows;
    logger.info(
      {
...
const updateAll = async (
  ...
  try {
    const [
      ...
      "UPDATE stock SET idcmp = ?, price = ?, availablequantity = ?, minimumquantity = ?, orderedquantity = ?, maximumquantity = ? where idstock = ?",
      [
        idcmp,
        price,
        availablequantity,
...
const updateForComponent = async (
  idcmp,
  price,
  availablequantity,
  minimumquantity,
  ...
  try {
    const [
      ...
      "UPDATE stock SET price = ?, availablequantity = ?, minimumquantity = ?, orderedquantity = ?, maximumquantity = ? where idcmp = ?",
      [
        price,
        availablequantity,
        minimumquantity,
...
const findOneByComponent = async (id) => {
  ...
  try {
    const [
      rows,
      fields,
    ] = await db.execute("SELECT * from site where idcmp = ? LIMIT 0, 1", [id]);
    result = rows;
  } catch (e) {
    // TODO return error
    logger.error(e);
...
const findOne = async (options) => {
  logger.debug(
    ...
    "will be returning stock for idstock:",
    options.stockId
  );
...
const findOneByComponentId = async (options) => {
  logger.debug(
    ...
    "will be returning stock for idcmp:",
    options.cmpId
  );
  ...
  try {
    const [
      ...
      "SELECT * from store.stock WHERE idcmp = ? LIMIT 0, 1",
      [options.cmpId]
    );
    result = rows;
  } catch (e) {
...
const findStocksComponentIds = async (options) => {
  logger.debug(
    ...
    "will be returning stocks for idcmps:",
    options.cmpIds
  );
...
const deactivate = async (id) => {
  ...
  try {
    const [
      rows,
      fields,
    ] = await db.execute("UPDATE stock SET isActive = 0 where idsite = ?", [
      id,
    ]);
    result = rows;
  } catch (e) {
...
const deactivateByComponent = async (id) => {
  ...
  try {
    const [
      rows,
      fields,
    ] = await db.execute("UPDATE stock SET isActive = 0 where idcmp = ?", [id]);
    result = rows;
  } catch (e) {
    // TODO return error
    logger.error(e);
