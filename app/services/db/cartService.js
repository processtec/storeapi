...
const modifyStatus = async (options) => {
    ...
    let result;
    try {
        const [rows, fields] = await db.query('UPDATE store.cart SET status = ? WHERE idcart = ? AND idUser', [options.status, options.idcart, options.idUser]);
        result = rows;
        logger.info({
            id: options.reqId,
            result: result
...
const addProductToCart = async (options) => {
    ...
    let result;
    try {
        const [rows, fields] = await db.query('INSERT INTO store.cart_stock SET idcart = ?, idstock = ?, quantity = ?, idUser = ?', [options.cartId, options.stockId, options.quantity, options.idUser]);
        result = rows;
        logger.info({
            id: options.reqId,
            result: result
...
const modifyProductForCart = async (options) => {
    ...
    let result;
    try {
        const [rows, fields] = await db.query('UPDATE store.cart_stock SET quantity = ? WHERE idcart = ? AND idstock = ?', [options.quantity, options.cartId, options.stockId, ]);
        result = rows;
        logger.info({
            id: options.reqId,
            result: result
    ...
    try {
        ...
        for (let index = 0; index < stocks.length; index++) {
            ...
            if (products.length < stock.quantity) {
                ...
                throw new Error("not enough product available for component: "+ stock.details.idcmp);
            }

            // 4.  Enter in transaction table the stockID
            const transaction = await createTransactionTx(connection, options, stock, cart);
            ...
            for (let j = 0; j < products.length; j++) {
                ...
                const updatedProduct = await markAProductSold(connection, options, product);
                ...
                await createTransactionDetailsTx(connection, options, product, transaction, cart);
...
const deleteAProduct = async (options) => {
    ...
    let result;
    try {
        const [rows, fields] = await db.query('DELETE FROM store.cart_stock WHERE idcart = ? AND idstock = ? AND idUser = ?', [options.cartId, options.stockId, options.userId]);
        result = rows;
        logger.info({
            id: options.reqId,
            result: result
...
const markAProductSold = async (connection, options, product) => {
    ...
    let result;
    try {
        const [rows, fields] = await connection.query('UPDATE store.product SET status = ?, lastModifiedBy = ?, isActive = 0 WHERE idproduct = ?', [SConst.PRODUCT.STATUS.SOLD, options.userName, product.idproduct]);
        result = rows;
        logger.info({
            id: options.reqId,
            result: result
...
const deleteCartProductsForAUserTx = async (connection, options) => {
    ...
    let result;
    try {
        const [rows, fields] = await connection.query('DELETE FROM store.cart_stock where idUser = ? ', [options.userId]);
        result = rows;
        logger.info({
            id: options.reqId,
            result: result
...
const deleteCartsForAUserTx = async (connection, options) => {
    ...
    let result;
    try {
        const [rows, fields] = await connection.query('DELETE FROM store.cart where idUser = ?', [options.userId]);
        result = rows;
        logger.info({
            id: options.reqId,
            result: result
...
const deleteCartTx = async (connection, options) => {
    ...
    let result;
    try {
        const [rows, fields] = await connection.query('DELETE FROM store.cart where idcart = ? AND idUser = ?', [options.cartId, options.userId]);
        result = rows;
        logger.info({
            id: options.reqId,
            result: result
        }, "cart deleted.");    } catch (e) {
        // TODO return error
        logger.error(e);
        throw e;
    } finally {
...
const markCartToStockInActiveTx = async (connection, options) => {
    ...
    let result;
    try {
        const [rows, fields] = await connection.query('UPDATE store.cart_stock SET isActive = 0 where idcart = ? ', [options.cartId]);
        result = rows;
        logger.info({
            id: options.reqId,
            result: result
...
const markCartInActiveTx = async (connection, options) => {
    ...
    let result;
    try {
        const [rows, fields] = await connection.query('UPDATE store.cart SET isActive = 0, status = ? where idcart = ? ', [SConst.CART.STATUS.COMPLETED, options.cartId]);
        result = rows;
        logger.info({
            id: options.reqId,
            result: result
